...
bool SX1302Interface::validateRadioConfig(uint8_t &halBandwidth) const
{
    ...
    case 500:
        ...
        break;
    default:
        LOG_ERROR("SX1302 supports only 125, 250, or 500 kHz LoRa bandwidth; "
                  "requested %.3f kHz",
                  bw);
        return false;
    }
...
void SX1302Interface::applyPowerLimit()
{
    ...
    if (safeMax < minimumPower) {
        LOG_ERROR("SX1302_MAX_POWER %d dBm is below the calibrated LUT minimum %d "
                  "dBm; disable transmit",
                  safeMax, minimumPower);
        disabled = true;
        txDisabledForPower = true;
        return;
    }
    // The LUT floor moves with the band, so the ceiling that was impossible on
    // one region can be fine on the next. Undo only our own latch - disable() is
    // a separate, deliberate one.
    if (txDisabledForPower) {
        LOG_INFO("SX1302_MAX_POWER %d dBm now clears the calibrated LUT minimum %d "
                 "dBm; re-enable transmit",
                 safeMax, minimumPower);
        disabled = false;
...
void SX1302Interface::resetConcentrator()
{
    if (!portduino_config.lora_reset_pin.enabled || portduino_config.lora_reset_pin.pin == RADIOLIB_NC) {
        LOG_WARN("SX1302 Reset GPIO is not configured; relying on an already-reset "
                 "concentrator");
        return;
    }
...
bool SX1302Interface::configureHardware()
{
    ...
    LOG_INFO("SX1302 HAL has no packet-in-progress CAD API; using Meshtastic "
             "randomized transmit scheduling");
    return true;
}
...
void SX1302Interface::clampToLateRebroadcastWindow(NodeNum from, PacketId id)
{
    ...
    packet->tx_after = millis() + getTxDelayMsecWeightedWorst(packet->rx_snr);
    bool dropped = false;
...
void SX1302Interface::setTransmitDelay()
{
    ...
    if (packet->tx_after) {
        const uint32_t addedDelay = packet->rx_rssi != 0 ? getTxDelayMsecWeighted(packet) : getTxDelayMsec();
        ...
        const uint32_t delay = (packet->rx_snr == 0 && packet->rx_rssi == 0) ? getTxDelayMsec() : getTxDelayMsecWeighted(packet);
...
void SX1302Interface::pollTransmit()
{
    ...
    if (txStartedAt && !Throttle::isWithinTimespanMs(txStartedAt, txTimeoutMsec)) {
...
void SX1302Interface::pollReceive()
{
    ...
    if (count < 0) {
        const uint32_t now = millis();
        if (!lastReceiveErrorAt || !Throttle::isWithinTimespanMs(lastReceiveErrorAt, 5000)) {
            LOG_WARN("lgw_receive failed; will keep polling");
            lastReceiveErrorAt = now;
        }
        // Recovery tears the interface down and re-runs initLoRa(), which for this
        // driver exits the process if it fails. A single failed SPI read must not
        // be worth that, so only ask for it once the concentrator has been failing
        // without interruption for RECOVERY_AFTER_MSEC.
        static constexpr uint32_t RECOVERY_AFTER_MSEC = 5000;
        if (++consecutiveReceiveErrors == 1) {
            firstReceiveErrorAt = now;
        } else if (!portduino_status.LoRa_in_error && !Throttle::isWithinTimespanMs(firstReceiveErrorAt, RECOVERY_AFTER_MSEC)) {
...
int32_t SX1302Interface::runOnce()
{
    ...
    if (!sendingPacket && !txQueue.empty()) {
        ...
        if (nextTxAt && static_cast<int32_t>(millis() - nextTxAt) >= 0) {
            nextTxAt = 0;
